...
| 7 | Scale-up 100k → 1M, batch tuning, `agg_alerts_by_site_month` | ✅ PASSED | `verify 7` — 29/29 checks, monthly 1m run 10.0 min at 8.38 GB peak (53 min from nothing), 33,355 alerts, 500 CRITICAL / 5,000 HIGH exactly | 2026-09-01 | `phase-07` |
...
The detector works at a million employees: 33,355 alerts, 500 CRITICAL and 5,000 HIGH exactly,
every one of the 34 codes still found, every bundle validated, 8.38 GB peak against a 12 GB budget,
and the map's 24 frames written and reconciled to the riyal. **A monthly batch — one new month of
data, every employee re-scored — is 10.0 minutes**; a store built from nothing is 53, and the gate
reports both. Everything phase 8 reads (`alerts.parquet` and `agg_alerts_by_site_month.parquet`,
under `data/runs/scale=<n>/run_id=<id>/`) exists at all three tiers.
